...
public class RouteItem {
    ...
    private int[] mVBOIdArray = new int[2];
    private int[] mVAOIdArray = new int[1];

    public RouteItem(float scaleSize, Context context) {
        mContext = context;
    ...
    private void initVertexData(float scaleSize) {
        ...
        try {
            InputStream is = mContext.getAssets().open("model/route_item/data.txt");
...
//                Log.d(ConstantUtil.TAG, str + "");
    ...
    private void initScript() {
        mProgram = GLES30Util.loadProgram(mContext, "model/route_item/script/vertex.shader", "model/route_item/script/fragment.shader");
        //获取程序中顶点位置属性引用
    ...
    private void initVAO() {
        //VAO
        GLES30.glGenVertexArrays(mVAOIdArray.length, mVAOIdArray, 0);
        GLES30.glBindVertexArray(mVAOIdArray[0]);

        //VBO
        GLES30.glGenBuffers(mVBOIdArray.length, mVBOIdArray, 0);
        GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, mVBOIdArray[0]);
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, mPositionList.size() * 4, mPositionBuffer, GLES30.GL_STATIC_DRAW);
        GLES30.glBindBuffer(GLES30.GL_ELEMENT_ARRAY_BUFFER, mVBOIdArray[1]);
        GLES30.glBufferData(GLES30.GL_ELEMENT_ARRAY_BUFFER, mIndexList.size() * 2, mIndexBuffer, GLES30.GL_STATIC_DRAW);

        //将顶点位置数据传入渲染管线
        GLES30.glVertexAttribPointer(maPositionHandle, 3, GLES30.GL_FLOAT, false, 0, 0);
        //将顶点法向量数据送入渲染管线
        GLES30.glVertexAttribPointer(maNormalHandle, 3, GLES30.GL_FLOAT, false, 0, 0);
    ...
    public void drawSelf() {
        ...
        //以三角形方式执行绘制
        GLES30.glDrawElements(GLES30.GL_TRIANGLES, mIndexList.size(), GLES30.GL_UNSIGNED_SHORT, 0);
//        GLES30.glDrawElements(GLES30.GL_LINE_LOOP, mIndexList.size(), GLES30.GL_UNSIGNED_SHORT, 0);
    }
}
