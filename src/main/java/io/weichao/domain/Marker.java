...
import io.weichao.model.ARPOICameraModel;
...
public class Marker implements Comparable<Marker> {
    ...
    private volatile static ARPOICameraModel cam = null;
    ...
    public synchronized void update(Canvas canvas, float addX, float addY) {
        ...
        if (cam == null) {
            cam = new ARPOICameraModel(canvas.getWidth(), canvas.getHeight(), true);
        }
        cam.set(canvas.getWidth(), canvas.getHeight(), false);
        cam.setViewAngle(ARPOICameraModel.DEFAULT_VIEW_ANGLE);
        populateMatrices(cam, addX, addY);
        updateRadar();
        updateView();
    }

    private synchronized void populateMatrices(ARPOICameraModel cam, float addX, float addY) {
        if (cam == null) {
            throw new NullPointerException();
        }
    ...
    private synchronized void updateRadar() {
        ...
        symbolXyzRelativeToCameraView.get(symbolArray);
        if ((symbolArray[2] < -1f) && ((x * x + y * y) < (radius * radius))) {
            isOnRadar = true;
        }
    }
    ...
    protected synchronized void drawCollisionZone(Canvas canvas) {
        ...
        float currentAngle = MathUtil.getAngle(symbolArray[0], symbolArray[1], textArray[0], textArray[1]) + 90;

        if (collisionPosition == null) {
            collisionPosition = new PaintablePosition(collisionBox, x1, y1, currentAngle, 1);
        } else {
            collisionPosition.set(collisionBox, x1, y1, currentAngle, 1);
        }
