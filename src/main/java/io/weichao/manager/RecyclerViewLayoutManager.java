...
public class RecyclerViewLayoutManager extends LayoutManager {
    private int mFirstPosition;
    private final int mScrollDistance;

    public RecyclerViewLayoutManager() {
        mScrollDistance = (int) (80 * BaseFragmentActivity.density + 0.5f);
    }

    @Override
    public RecyclerView.LayoutParams generateDefaultLayoutParams() {
        // return new RecyclerView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
        // ViewGroup.LayoutParams.WRAP_CONTENT);//会导致前几个子控件不能居中
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dy < 0) {
            ...
            while (scrolled < dy) {
                ...
                final int hangingBottom = Math.max(getDecoratedBottom(bottomView) - parentHeight, 0);
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (direction == View.FOCUS_DOWN || direction == View.FOCUS_FORWARD) {
            while (mFirstPosition + getChildCount() < state.getItemCount() && newViewsHeight < mScrollDistance) {
                View v = recycler.getViewForPosition(mFirstPosition + getChildCount());
                final int top = getDecoratedBottom(getChildAt(getChildCount() - 1));
                addView(v);
                measureChildWithMargins(v, 0, 0);
    ...
    public void recycleViewsOutOfBounds(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (v.hasFocus() || (getDecoratedRight(v) >= 0 && getDecoratedLeft(v) <= parentWidth && getDecoratedBottom(v) >= 0 && getDecoratedTop(v) <= parentHeight)) {
                if (!foundFirst) {
                    first = i;
                    foundFirst = true;
                }
